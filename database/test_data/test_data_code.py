...
while restart_count < 5 and has_errors:
    ...
    try:
        if not -34569849040231179 in succeeded_hashes:  # avoid duplicate inserts
            instance = personnel1 = Personnel(id=1, first_name='John', last_name='Doe', email='john.doe@example.com', phone_number='123-456-7890')
            session.add(instance)
            session.commit()
            succeeded_hashes.add(-34569849040231179)
    except Exception as e:
        has_errors = True
